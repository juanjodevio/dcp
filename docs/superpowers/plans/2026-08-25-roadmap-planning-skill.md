...
Read [MILESTONE-TEMPLATES.md](MILESTONE-TEMPLATES.md) before planning or mutating Linear. Its authoring forms, parsing patterns, sync-key exactness rules, and milestone activity rules are binding.
...
5. Parse milestone and deliverable identifiers with the required parsing patterns in MILESTONE-TEMPLATES.md. Accept em dash, en dash, and hyphen-minus milestone separators, and accept indented `-`, `*`, and `+` deliverable bullets. Never silently omit a line that advertises an identifier but fails its pattern; report it and block.
...
1. Never commit or push to `main`. Create a branch from the resolved authority commit named `chore/roadmap-link-<short-approved-roadmap-sha>-<run-id>`.
2. Add only `Linear tickets:` reference lines for the milestones whose tickets this run created. Change nothing else: no milestone or deliverable additions, removals, renames, or reordering, no wording edits, no steering or ADR edits, and no application code.
3. Re-read the resulting diff and confirm every added or changed line is a `Linear tickets:` reference line. If any other line changed, discard the branch, open no pull request, and report BLOCKED.
4. Open a pull request targeting `main` and leave it for human merge. Never merge it, never enable auto-merge, and never push to `main`.
5. If GitHub is unavailable, unconfigured, or the pull request cannot be created, keep the branch local and unmerged, return PARTIAL when Linear synchronization otherwise succeeded and BLOCKED when it did not, and state the exact setup action, such as adding a GitHub `origin` remote, authenticating the `gh` CLI, and then opening a pull request from the recorded branch into `main`.

Record the branch name, the pull request URL or its absence, and the exact human action under `### Roadmap link pull request`.
...
**Interfaces:**
- Consumes: Cursor skill and subagent discovery plus the thirteen roadmap dry-run fixtures
...
- `origin/main` divergence or fetch failure blocks before any read of approved intent or Linear mutation; and
- an unavailable forge keeps the roadmap-link branch local, never commits to `main`, and reports the exact setup action.
