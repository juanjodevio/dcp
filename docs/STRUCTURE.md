...
This repository will hold the **dcp** product (modular monolith) plus durable steering docs and agent-delivery skills. Application packages under `control-plane/` are **planned** and not present yet. Existing tree is mostly docs and planning artifacts.
...
Planned: colocate or mirror under `tests/` (exact layout **Unknown** until scaffold). Prefer fixtures with sample dbt `manifest.json` / `run_results.json` under a dedicated fixtures directory—not production object storage.
...
- Exact Python package layout (`src/` vs flat) under uv/`pyproject.toml`, and UI package path if monorepo tools require `apps/web`
